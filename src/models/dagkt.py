"""Dual Attention Graph Knowledge Tracing modules"""
import numpy as np
import torch as th
...
#             subg.update_all(fn.copy_u('x', 'm'), self._node_attention)

#     def _node_attention(self, nodes):
#         self.subg_rep
...
class DAGKT(nn.Module):
    ...
    def __init__(self, in_nfeats, in_efeats, latent_dim,
        ...
        self.convs.append(EGATConv(in_node_feats=in_nfeats,
        ...
        self.node_attention_layer = nn.Linear(3*latent_dim[0], 1) 
    ...
    def forward(self, graph):
        ...
        for conv in self.convs:
            ...
            graph.ndata['x'] = x
            subgs = dgl.unbatch(graph)

            # TODO: save node importance
            ...
            for subg in subgs:
                ...
                g = th.cat([subg_rep.repeat(num_node,1), subg_x], dim=1)
                # print(g.shape)
                g = self.node_attention_layer(g)
                g = self.softmax(self.leakyrelu(g))
                subg_x = subg_x*g
                importance_applied_features.append(subg_x)
                # subg.ndata['x'] = subg_x

            x = th.cat(importance_applied_features, 0)
            states.append(x)
